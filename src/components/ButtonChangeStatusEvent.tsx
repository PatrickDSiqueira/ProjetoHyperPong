...
const ButtonChangeStatusEvent = ({statusSelected}:Props) => {
    ...
    const handleClickToChangeStatus = async (status: number) => {
        ...
        await axios.post(`${process.env.REACT_APP_BACKEND}api/admin/events/${id}/status/${status}`)
    ...
    return <>
        ...
        {!visibleLoading && <ContainerButtonChangeStatusEvent>
            ...
            <>
                <Button className={(selectedStatus === 0)?"selection":""} status={0} onClick={() => handleClickToChangeStatus(0)}>{StatusEvents[0]}</Button>
                <Button className={(selectedStatus === 1)?"selection":""} status={1} onClick={() => handleClickToChangeStatus(1)}>{StatusEvents[1]}</Button>
                <Button className={(selectedStatus === 2)?"selection":""} status={2} onClick={() => handleClickToChangeStatus(2)}>{StatusEvents[2]}</Button>
                <Button status={3} onClick={() => setClickDeleteEvent(!clickDeleteEvent)} className={!clickDeleteEvent ? "" : "hidden"}>Apagar Evento</Button>
            </>
            <div className={clickDeleteEvent ? "show" : "hidden"}>
                <p>Deseja mesmo apagar este evento?</p>
                <ButtonConfirmation status={1} onClick={() => setClickDeleteEvent(!clickDeleteEvent)}>Cancelar</ButtonConfirmation>
